...
class Model_Auth extends CI_Model {
    function auth_kaprodi($username,$password){
        $query=$this->db->query("SELECT * FROM tbl_kaprodi WHERE id='$username' AND pass=MD5('$password') LIMIT 1");
        return $query;
    }
    
    function auth_baak($username,$password){
        $query=$this->db->query("SELECT * FROM tbl_baak WHERE id='$username' AND pass=MD5('$password') LIMIT 1");
        return $query;
    }
